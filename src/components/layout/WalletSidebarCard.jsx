...
const WalletSidebarCard = ({ className, isVisible = true, onNavigate }) => {
  ...
  return (
    <section
      dir={dir === 'rtl' ? 'rtl' : 'ltr'}
      className={cn(
        'relative isolate overflow-hidden rounded-[15px] border border-[color:rgb(var(--color-primary-rgb)/0.34)] bg-[linear-gradient(145deg,rgb(var(--color-primary-rgb)/0.14),rgba(217,119,6,0.1)_40%,rgb(var(--color-card-rgb)/0.92)_100%)] p-2 shadow-[0_12px_28px_-22px_rgb(var(--color-primary-rgb)/0.36)]',
        'before:pointer-events-none before:absolute before:inset-0 before:bg-[radial-gradient(circle_at_top_right,rgba(245,158,11,0.22),transparent_45%)] before:opacity-80',
        className
      )}
    >
      <div className="relative z-10 space-y-1.5">
        <div className="flex items-start justify-between gap-2">
            ...
            <div className="mt-0.5 min-h-[1.35rem]">
              {(!user && isRefreshing) ? (
                <div className="space-y-1">
                  ...
                  <div className="h-4 w-24 animate-pulse rounded-full bg-[color:rgba(245,158,11,0.18)]" />
                </div>
              ) : (
          ...
          <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-[color:rgb(var(--color-primary-rgb)/0.4)] bg-[linear-gradient(145deg,rgba(180,83,9,0.9),rgba(217,119,6,0.95))] text-[#fef08a] shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_0_24px_-10px_rgba(217,119,6,0.85)]">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          ...
          <button
            ...
            className="inline-flex h-7 items-center justify-center gap-1 rounded-[10px] border border-[color:rgb(var(--color-primary-rgb)/0.45)] bg-[linear-gradient(135deg,#b45309_0%,#d97706_50%,#f59e0b_100%)] px-1.5 text-[9px] font-bold text-white shadow-[0_0_26px_-16px_rgba(217,119,6,0.85)] transition-colors hover:brightness-[1.1]"
          >
